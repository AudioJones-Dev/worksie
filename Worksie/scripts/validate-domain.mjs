...
const checks = [
  ...
  {
    name: 'Invoices can reconcile to weekly billing batches within the same organization',
    ok: schema.includes('billingBatchId String?')
      && schema.includes('billingBatch WeeklyBillingBatch? @relation(fields: [organizationId, billingBatchId], references: [organizationId, id]')
      && migration.includes('FOREIGN KEY ("organizationId", "billingBatchId") REFERENCES "WeeklyBillingBatch" ("organizationId", "id")'),
  },
  {
